...
training_out = pd.DataFrame(columns=["tokenized_method","tokenized_target","input_ids"])
testing_out = pd.DataFrame(columns=["tokenized_method","tokenized_target","input_ids"])
validating_out = pd.DataFrame(columns=["tokenized_method","tokenized_target","input_ids"])
...
with alive_bar(training.shape[0]+testing.shape[0]+validating.shape[0]) as bar:
    ...
    for frame in range(len(processed_data)):
        ...
        for index, row in processed_data[frame].iterrows():
            ...
            new_row = [tokenized_method, tokenized_target, [embedded_method, embedded_target]]
